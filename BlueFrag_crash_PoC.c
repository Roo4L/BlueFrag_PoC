...
int main(int argc,char **argv) {
    ...
    uint8_t packet[4 + datalen + 0x1000]; // Why this length of packet ??? (especially the last argument)
    memcpy(&packet[0],&_bs_l2cap_len,2);
    memcpy(&packet[2],&_bs_cid,2);
    memset(&packet[4], 0x99, datalen+0x1000);
    int fl = 36; // WHat vaiable means
    int i =0 ;
...
int hci_send_acl_data(int hci_socket, uint16_t hci_handle, uint8_t *data, uint16_t data_length, uint16_t PBflag, uint16_t dlen){
    ...
    */
    uint16_t BCflag = 0x0000;               // Broadcast flag (WHAT IS THIS?)
    ...
    //hd.dlen = (data_length);
    hd.dlen = dlen;
    struct iovec iv[3]; // An iovec structure describes a data storage area for transfer in a uio(9S) structure. Conceptually, it can be thought of as a base address and length specification.  
    /*
    ...
    */
    ...
    while (writev(hci_socket, iv, ivn) < 0) { // Функция writev() записывает по меньшей мере count блоков, описанных вектором vector в файл, связанный с описателем файла fd.
